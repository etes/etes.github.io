...
define(["dojo/ready", "dojo/json", "dojo/_base/array", "dojo/_base/Color", "dojo/_base/declare", "dojo/_base/lang", "dojo/dom", "dojo/dom-geometry", "dojo/dom-attr", "dojo/dom-class", "dojo/dom-construct", "dojo/dom-style", "dojo/on", "dojo/Deferred", "dojo/promise/all", "dojo/query", "dijit/registry", "dijit/Menu", "dijit/CheckedMenuItem", "application/toolbar", "application/has-config", "esri/arcgis/utils", "esri/lang", "esri/dijit/HomeButton", "esri/dijit/LocateButton", "esri/dijit/Legend", "esri/dijit/BasemapGallery", "esri/dijit/Measurement", "esri/dijit/OverviewMap", "esri/geometry/Extent", "esri/layers/FeatureLayer", "application/TableOfContents", "application/ShareDialog", "application/esriJson2Excel", "application/vendor/download", "application/search"], function(ready, JSON, array, Color, declare, lang, dom, domGeometry, domAttr, domClass, domConstruct, domStyle, on, Deferred, all, query, registry, Menu, CheckedMenuItem, Toolbar, has, arcgisUtils, esriLang, HomeButton, LocateButton, Legend, BasemapGallery, Measurement, OverviewMap, Extent, FeatureLayer, TableOfContents, ShareDialog, esriJson2Excel, Download, Search) {
	...
	return declare(null, {
		...
		startup : function(config) {
			...
			if (config) {
				...
				ready(lang.hitch(this, function() {
					...
					this.esriJson2Excel = new esriJson2Excel();
		...
		_createUI : function() {
			...
			toolbar.startup().then(lang.hitch(this, function() {
				...
				all(toolList).then(lang.hitch(this, function(results) {
					...
					if (has("download")) {
						downloadNode = domConstruct.create("div", {
							className : "panelTool",
							id : "btnDownload",
							innerHTML : "<img class='tool' src='images/icons_white/download.png'>"
						}, dom.byId("panelTools"));

						on(downloadNode, "click", lang.hitch(this, function() {

							var downloadConfig = this.config.download;
							downloadLayer = typeof downloadConfig != "object" ? JSON.parse(downloadConfig) : downloadConfig;
							mapLayer = this.map.getLayer(downloadLayer.layerId);
							queryParams = "query?where=1=1&returnGeometry=true&spatialRel=esriSpatialRelIntersects&outFields=*&outSR=4326&f=json";
							queryURL = mapLayer.url + '/' + queryParams;
							esriJson2Excel = this.esriJson2Excel;
							dojo.when(esriJson2Excel.getJSON(queryURL), function(result) {
								var items = [];
								dataTypes = {};
								for (var i = 0; i < result.features.length; i++) {
									items.push(result.features[i].attributes);
								};
								for (var i = 0; i < result.fields.length; i++) {
									name = result.fields[i].name;
									dataTypes[result.fields[i].name] = result.fields[i].type;
								};
								excelData = esriJson2Excel.jsonToSsXml(items, dataTypes);
								//Create a file name and initialize xls file format for downloading
								Download(excelData, 'skjenkebevillinger.xls', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
							});
						...
						if (!has("touch")) {
							//add a tooltip
							domAttr.set("btnDownload", "data-title", this.config.i18n.tooltips.download);
						} else {
		...
		_createMapUI : function() {
			...
			require(["application/has-config!customSearch?application/search"], lang.hitch(this, function(Search) {
				if (!Search) {
				...
				var options = {
					map : this.map,
					searchConfig: this.config.customSearch,
					maxSuggestions : 6
				};
		...
		_addBasemapGallery : function(tool, toolbar, panelClass) {
			...
			if (has("basemap")) {
				...
				var basemap = new BasemapGallery({
					...
					basemapsGroup : {
						"id" : "de2f7dc193f34833ae2ed1143c133ce9",
						"title" : "Bakgrunnskart-enkelt",
						"owner" : "ermtes"
					}
		...
		_createWebMap : function(itemInfo) {
			...
			arcgisUtils.createMap(itemInfo, "mapDiv", {
				...
				//remove Esri logo
				query(".logo-med").style("visibility", "hidden");
				query(".logo-sm").style("visibility", "hidden");
